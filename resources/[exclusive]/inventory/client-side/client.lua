...
AddEventHandler("inventory:stealTrunk",function(entity)
   	if useWeapon == "WEAPON_CROWBAR" then
		...
		TriggerEvent("Notify","negado","<b>Pé de Cabra</b> não encontrado.",10000,"bottom")
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ENTERVEHICLE
...
RegisterCommand("moc",function(source,args)
	if GetEntityHealth(PlayerPedId()) > 101 and not blockButtons and not LocalPlayer.state.inArena then
...
RegisterCommand("+tabhotbar",function(source,args)
	if not LocalPlayer.state.inArena then 
		local inventory = vSERVER.getUserHotkeys()
		if inventory then 
			SendNUIMessage({ action = "interactHotbar", inventory = inventory, show = true })
		end
	end
end)

RegisterCommand("-tabhotbar",function(source,args)
	SendNUIMessage({ action = "interactHotbar", show = false })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- KEYMAPPING
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterKeyMapping("moc","Abrir a mochila","keyboard","oem_3")
RegisterKeyMapping("+tabhotbar","Mostrar hotbar","keyboard","tab")
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADFOCUS
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
...
function cRP.putWeaponHands(weaponName,weaponAmmo)
	if not putWeaponHands then
		...
		if not IsPedInAnyVehicle(ped) then
			...
			exports.player:applyAttachs(weaponName)
		...
		TriggerEvent("hud:showWeapon", weaponName)
...
function cRP.storeWeaponHands()
	if not storeWeaponHands then
		...
		weaponActive = false
		TriggerEvent("hud:showWeapon", false)
		useWeapon = ""
...
local adrenalineCds = {
	...
	{ 1604.31,3569.39,38.77 },
	{ 1597.51,3581.31,38.77 },
	{ -469.52,6287.29,13.61 },
	{ 1795.38,107.42,173.86 },
	{ 1795.27,107.93,176.79 },
	{ 319.92,-560.05,28.75 },
	{ 343.77,-1398.65,32.5 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	while true do
		...
		if dropList[gridZone]then
			for k,v in pairs(dropList[gridZone]) do
				if v["session"] == LocalPlayer.state.session then 
					...
					if distance <= 50 then
						...
						DrawMarker(21,v["coords"][1],v["coords"][2],v["coords"][3] + 0.25,0.0,0.0,0.0,0.0,180.0,0.0,0.20,0.20,0.20,133,141,235,125,0,0,0,1)
					end
				end
			end
		end
...
AddEventHandler("inventory:removeTyres",function(Entity)
	if GetVehicleDoorLockStatus(Entity[3]) == 1 then
		if useWeapon == "WEAPON_WRENCH" then
			local ped = PlayerPedId()
			local coords = GetEntityCoords(ped)
			...
			TriggerEvent("Notify","negado","<b>Chave Inglesa</b> não encontrada.",5000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
