	...
	window.addEventListener("message", function (event) {
		switch (event["data"]["action"]) {
			case "showMenu":
				...
				break;
			case "interactHotbar":
				if (!event["data"]["show"]) return $('.slots-container').hide();
				updateHotkeys(event["data"]["inventory"]);
				break;
			case "showWeapon":
				showWeapon(event["data"])
				break;
			case "closeWeapon":
...
const showWeapon = (data) => {
	let ammo = data.infoGun.weaponsInfo.maxammo
	$('.userGun-container').fadeIn(400).css({'display':'flex'});
	// $('.userGun-container').css('background-image', `url(http://181.215.253.160/img/${data.infoGun.weaponsInfo.index}.png)`);
	$('.userGun-container').html(`
		${
			data.infoGun.weaponsInfo.maxammo <= 0 
			|| data.infoGun.weaponsInfo.index == "stungun" ? 
			`
				<i class="fa-solid fa-gun-slash"></i>
				<p>...</p> 
				<span>...</span>
			` 
			: `
				<i class="fa-duotone fa-gun"></i>
				<p>${ammo}</p> 
				<span>${data.infoGun.weaponsInfo.inclip}</span>
		`}
	`)
}

const updateHotkeys = (inventory) => {
	let container = document.querySelector('.slots-container')
	$('.slots-container').show()
	container.innerHTML = `
	<div class='slot slot-1' data-number="1"></div>
	<div class='slot slot-2' data-number="2"></div>
	<div class='slot slot-3' data-number="3"></div>
	<div class='slot slot-4' data-number="4"></div>
	<div class='slot slot-5' data-number="5"></div>
	`
	for (let i in inventory) {
		$(`.slot-${inventory[i].pos}`).html(`
			<div class="imgSlot" style="background-image: url(http://localhost/baixada/item/${inventory[i].item.split('-')[0]}.png)"></div>
			<p>${inventory[i].itemname.split('-')[0]}</p>
		`);
	}
}
...
const updateDrag = () => {
	...
	$(".populated").tooltip({
		create: function (event, ui) {
			...
			var max = $(this).attr("data-charges") || $(this).attr("data-max");
        ...
        <div class='tooltip'>
            ...
            <div class='info-item'>
                ...
                <div class='info'>${plate !== "undefined" ? plate : `${(weight * amounts).toFixed(2)}kg`}</div>
            </div>
        </div>`,
