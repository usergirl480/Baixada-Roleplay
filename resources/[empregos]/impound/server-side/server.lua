	...
	if user_id then
		...
		if vehicle then
			if impoundVehs[vehName.."-"..vehPlate] == nil then
				...
				TriggerClientEvent("Notify",source,"sucesso","Obrigado(a) por colaborar com a cidade!",10000,"bottom")
...
RegisterCommand("placa",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if exports["common"]:Group().hasPermission(user_id,"police") or exports["common"]:Group().hasPermission(user_id,"admin") and args[1] then
...
function runPlate(source,vehPlate)
	...
	if plateUser then
		...
		if identity then
			vRPC.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
			TriggerClientEvent("Notify",source,"default","<b>Passaporte:</b> "..identity["id"].."<br><b>Nome:</b> "..identity["name"].." "..identity["name2"].."<br><b>Nº:</b> "..identity["phone"].."",10000,"normal","atenção")
		end
	else
		...
		vRPC.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
		TriggerClientEvent("Notify",source,"default","<b>Passaporte:</b> 9.999<br><b>Nome:</b> "..plateSave[vehPlate][1].."<br><b>Nº:</b> "..plateSave[vehPlate][2].."",10000,"normal","atenção")
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- POLICE:RUNARREST
