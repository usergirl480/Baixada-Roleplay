...
AddEventHandler("towdriver:callPlayers",function(source,vehName,vehPlate)
	...
	for k,v in pairs(userList) do
		async(function()
			TriggerClientEvent("NotifyPush",v,{ code = "QTH", title = "Registro de Veículo", x = coords["x"], y = coords["y"], z = coords["z"], vehicle = vehicleName(vehName).." - "..vehPlate, time = "Recebido às "..os.date("%H:%M"), blipColor = 77 })
		end)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
