			...
			if IsPedInAnyVehicle(ped) then
				...
				if IsControlJustPressed(1,306) then
					if circuitInit then
						...
						TriggerEvent("Notify","sucesso","Inicio marcado.",3000)
					else
				...
				elseif IsControlJustPressed(1,301) then
					if #myCircuit["coords"] > 0 then
						...
						TriggerEvent("Notify","sucesso","Circuito finalizado.",3000)
						vSERVER.inputRaces(myCircuit)
						makeCircuits = false
					else
						TriggerEvent("Notify","importante","É obrigatório ter no mínimo <b>1 Checkpoint</b>.",3000)
					end
				end
			end
		end
