...
-- APPLYTIMERS
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.checkSystems(atmId)
	local source = source
	...
	if user_id then
		...
		-- if reputationResult >= 150 then
            ...
            if atmTimers[atmId] then
                if GetGameTimer() < atmTimers[atmId] then
                    TriggerClientEvent("Notify",source,"negado","Este Caixa Eletrônico já foi roubado.",10000,"bottom")
                    return false
                else
                    startAtm(atmId,source)
                    return true
                end
            else
                startAtm(atmId,source)
...
-- STARTBOX
-----------------------------------------------------------------------------------------------------------------------------------------
function startAtm(atmId,source)
    local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
        atmTimers[atmId] = GetGameTimer() + (20 * 60000)
...
function cRP.paymentSystems()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		exports["wanted"]:setWanted(user_id,3)
		vRP.generateItem(user_id,"dollarsz",math.random(1,25),true)
	end
end