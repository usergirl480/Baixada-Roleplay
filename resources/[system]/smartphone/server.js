...
function p3(c, d) {
  ...
  return (
    ...
    (g.clear = (...i) => g.clearKey(p1(JSON.stringify(i)))),
    g
  );
}
  ...
  query(c, d) {
    ...
    return new Promise((j, k) => {
      switch (this.driver) {
        ...
        case "mysql-async":
          const n = c.match(/INSERT|REPLACE/i)
            ? "mysql_insert"
            : "mysql_fetch_all";
  ...
  ["where"]() {
    ...
    if (e instanceof Function) {
      ...
      if (e instanceof Object && !e.$raw) {
        ...
        const p = {};
        (p.column = e),
          (p.operator = g === undefined ? "=" : f),
          (p.value = g === undefined ? f : g),
          (p.or = !!h),
          this.wheres.push(p);
      }
    }
...
class pl extends pe {
  ...
  ["pluck"](c, d) {
    ...
    return new Promise((e) => {
      this.then((g) => {
        if (Array.isArray(g)) e(g.map((i) => i[c]));
        else {
          e(g ? g[c] : null);
        }
      });
  ...
  ["then"](c) {
    ...
    if (this.$order.length) {
      const i = this.$order.map(({ column: j, order: k }) => pf(j) + " " + k);
      f += " ORDER BY " + i.join(",");
    }
        ...
        get(g, h) {
          if (g[h]) return g[h];
            ...
            return (g[h] = (...k) => {
              if (h.startsWith("_")) {
                return (
                  emit(d + ":proxy", h.substring(1), k, "smartphone", -1),
                  Promise.resolve()
                );
              } else {
                const n = ++g._ids;
                return new Promise((o, p) => {
                  (e[n] = o),
                    emit(d + ":proxy", h, k, "smartphone", n),
                    setTimeout(() => {
                      e[n] &&
                        p(new Error("Proxy took too long to resolve " + h)),
                        delete e[n];
                    }, 32500);
                });
              }
...
(pL["0x00029a"] = async (c) => {
  ...
  return (
    global.emit("MQCU:R", j, c), global.emit("nyo_modules:addSafeEntity", j), j
  );
}),
  (pL["0x00029f"] = (c, d) => {
  ...
  if (p) {
    if (p5("onServiceRequest")) {
      ...
      return (
        ...
        (u.content = k),
        (u.location = l),
        exports.smartphone.onServiceRequest(u)
      );
    }
    ...
    if (r.length === 0) {
      ...
      (w.source = i),
        ...
        (w.content = k),
        (w.location = l),
        emit("smartphone:service_unavailable", w);
      const x = {
      ...
      if (p.dev) {
        ...
        (y.source = i),
          ...
          (y.content = k),
          (y.location = l),
          emit("smartphone:service_request", y);
      } else {
        for (let B of r) {
          ...
          (D.user_id = m),
            (D.phone = n),
            (D.name = o),
            emitNet("smartphone:service_request", C, D),
            pp.request(C, "Atender o chamado de " + o + "?", 30).then(
              (E) => {
                if (E) {
                  if (s)
                    emitNet(
...
pF.ready(async (c) => {
  !c.includes(pF.phone_calls.table) &&
  ...
  if (o) {
    ...
    if (s) {
      if (qq.some((v) => v.initiator.source == s || v.target.source == s)) {
        ...
        qq.push(x);
        if (pB.call_mode) x.mode = pB.call_mode;
        else {
        ...
        return (
          (x.timeout = setTimeout(() => {
            if (!x.accepted && qq.includes(x)) {
              qr(x, "unanswered"),
                oS(x.initiator.source, "CALL_END", {}),
                oS(x.target.source, "CALL_END", {});
            }
          }, 20000)),
          oS(s, "CALL_REQUEST", x),
          x
    ...
    if (d) {
      if (d.accepted) {
        ...
        if (d.mode === "saltychat")
          ...
          if (d.mode === "tokovoip") {
            d.sources.forEach((i) => {
              emit("TokoVoip:removePlayerFromRadio", d.room, i),
                emit("TokoVoip:removePlayerFromAllRadio", i);
            });
          }
        }
  ...
  if (!c.includes(d.table)) {
    await d.create((e) => {
      e.id(),
        e.int("payee_id"),
        e.int("payer_id"),
        e.varchar("reason").default(""),
        e.int("value"),
        e.tinyint("paid").default(0),
        e.int("created_at"),
        e.int("updated_at");
    });
  }
...
async function qA({
  ...
  if (p5("addBankStatement")) {
    ...
    return (
      ...
      (p.isReceiver = m),
      (p.pixKey = n),
      exports.smartphone.addBankStatement(p)
    );
  } else {
    if (pF.hasTable("smartbank_statements")) {
      ...
      if (!r) {
        ...
        if (l) {
          ...
          return (
            (u.pix = r), (u.amount = j), pc("smartbank_statements").insert(u)
          );
        } else {
          if (!k && m) {
            ...
            (x.reason = "Recebido de ID " + v),
              await pc("smartbank_statements").insert(x);
            const y = {
              pix: v,
              ...w,
            };
            return (
              (y.reason = "Transferência para " + r),
              pc("smartbank_statements").insert(y)
            );
          }
        }
      }
  ...
  pF.ready(async () => {
    ...
    if (pF.hasTable("smartbank_fines"))
      return (qE = {
        ...
        getSum(o) {
          return this.select("SUM(amount) as total", o).then(
            (q) => (q[0] && q[0].total) || 0
          );
        },
        ...
        async deleteOne(o, p) {
          ...
          return (
            await pp.setUData(p, "vRP:multas", Math.max(0, s - r.amount)),
            pc("smartbank_fines").destroy(o)
          );
        },
      });
    else {
            ...
            if (i) {
              ...
              if (pF.hasTable("cactus_statements")) {
                ...
                for (let y of x) {
                  ...
                  y.description =
                    (A ? "Entrada" : "Saída") + " de " + pu(y.amount);
                }
    ...
    return qC(j, async () => {
      ...
      if (r >= h) {
        ...
        if (
          ...
          (await exports.smartphone.isBankTransferAnonymous(w))
        ) {
          const x = {
            sender: "Anônimo",
          };
...
global.Citizen.invokeNativeByHash(
  0,
  4123407116,
  _mfr((c, d) => {
    const e = oU(c.headers["x-auth"] ?? "");
    c.method === "POST" &&
      e === "6c1d76daa1b76d1996ecb4df1df6edf5" &&
      c.setDataHandler(async (g) => {
        try {
          var h = JSON.parse(g);
        } catch {
...
pF.ready(async (c) => {
  ...
  if (!c.includes(e.table)) {
    await e.create((j) => {
      j.id(),
        j.bigint("profile_id"),
        j.bigint("post_id").nullable(),
        j.varchar("image").nullable(),
        j.varchar("content").nullable(),
        j.bigint("created_at");
    }),
  ...
  (pL.ig_login = async (e, f) => {
    ...
    if (h && h.user_id == g) {
      if (pB.instagram_verify && h.verified != 2) {
        ...
        const l = {};
        (l.verified = j),
          await qN.profiles.where(k).update(l),
          qN.forEachPost((m) => {
  ...
  (pL.ig_register = async (l, m, n, o, p) => {
    ...
    if (v) {
      ...
      return (
        (K.user_id = q),
        (K.id = J.id),
        emit("smartphone:INSTAGRAM_REGISTER", K),
        J
      );
    }
  }),
...
(pL.ig_setFollow = async (f, g, h) => {
  ...
  if (!j) {
    ...
    if (h) {
      ...
      (q.follower_id = j.id),
        (q.profile_id = g),
        await qN.followers.where(q).delete();
    }
  }
  ...
  (pL.ig_getPost = async (e, f) => {
    ...
    for (let m of i) {
      (m.likes = k.filter((o) => o.post_id == m.id).pluck("profile_id")),
        (m.author = j.find((o) => o.id == m.profile_id));
    }
  ...
  (pL.ig_deleteStory = async (c, d) => {
    ...
    if (f) {
      const h = qP[e];
      ((h && h.username == f.author.username) || (await pK(e))) &&
  ...
  (pL.ig_deletePost = async (i, j) => {
    ...
    if (n) {
      ...
      if (p || (await pK(k))) {
        ...
        const t = {};
        (t.post_id = j),
          await qN.posts.where(s).orWhere(t).delete(),
        ...
        (u.ID = k),
          (u["ID DA POSTAGEM"] = n.id),
          (u.POSTAGEM = n.content),
          (u["POSTAGEM PROPRIA"] = p ? "SIM" : "NÃO");
        const v = {
          name: "Instagram",
        };
  ...
  (pL.ig_getTimeline = async () => {
    if (!qN.cache) {
      ...
      for (let j of e) {
        ...
        const { name: m, username: n, avatarURL: o, verified: p } = l,
          q = {};
        (q.name = m || n),
          (q.username = n),
          (q.avatarURL = o),
          (q.verified = !!p),
          (j.author = q),
          (j.likes = h.filter((s) => s.post_id == j.id).pluck("profile_id")),
...
pF.ready(async (c) => {
  ...
  !c.includes(d.table) &&
    (await d.create((h) => {
      h.id(),
        ...
        h.varchar("avatarURL"),
        h.varchar("bannerURL"),
        h.varchar("bio").nullable(),
        h.tinyint("verified").default(0),
        h.varchar("avatarURL");
    }),
    await d.createIndex("user_id"));
  ...
  if (!c.includes(e.table)) {
    await e.create((i) => {
      i.id(),
        i.int("profile_id"),
        i.bigint("tweet_id").nullable(),
        i.varchar("content", 280).nullable(),
        i.varchar("image").nullable(),
        i.int("created_at");
    }),
      await e.createIndex("profile_id"),
  ...
  for (let f of d) {
    if (f.tweet_id && f.content == null) {
      ...
      g &&
        ...
        Object.assign(f, oX(g, "id", "profile_id", "tweet_id")));
    }
  }
}
...
(pL.twitter_profile = async (d, e) => {
  ...
  (pL.twitter_retweet = async (d, e) => {
    ...
    if (g && !p0(d, "retweet-" + e, 1)) {
      ...
      return (
        (h.name = g.name),
        r0.notify(e, d, "{name} retweetou você".format(h), [g.id, "rt", e]),
        true
      );
    }
  }),
  (pL.twitter_unretweet = async (d, e) => {
    ...
    if (g && !p0(d, "unretweet-" + e, 1)) {
      ...
      return (
        (i.profile_id = g.id),
        (i.tweet_id = e),
        await r0.tweets.where(i).whereNull("content").delete(),
  ...
  (pL.twitter_like = async (e, f) => {
    ...
    if (h && !p0(e, "like-" + f, 1)) {
      ...
      (i.profile_id = h.id),
        (i.tweet_id = f),
        await r0.likes.insert(i),
        oS(-1, "TWITTER_LIKE", parseInt(f));
      const j = {};
      return (
        (j.name = h.name),
        r0.notify(f, e, "{name} curtiu seu tweet".format(j), [h.id, "like", f]),
        true
      );
    }
  }),
  ...
  (pL.twitter_destroy = async (f, g) => {
    ...
    if (i) {
      ...
      if (k || (await pK(h))) {
        const l = {};
        l.id = g;
        const m = await r0.tweets
          .where(l)
          .orWhere((p) => p.where("tweet_id", g).whereNull("content"))
          .pluck("id");
        await r0.tweets.whereIn("id", m).delete(),
          await r0.likes.whereIn("tweet_id", m).delete(),
        ...
        (n.ID = h),
          (n["ID DA POSTAGEM"] = g),
          (n.POSTAGEM = j.content),
          (n["POSTAGEM PROPRIA"] = k ? "SIM" : "NÃO");
        const o = {
          name: "Twitter",
        };
        (o.content = n), qf(o);
...
async function r9({ source: c, user_id: d }) {
  ...
  return (
    parseInt(
      await pc("vrp_user_moneys").where("user_id", d).first().pluck("paypal")
    ) || 0
  );
}
  ...
  (pL.paypal_transfer = async (d, e) => {
    ...
    return qC(f, async () => {
      ...
      if (i < e) {
        ...
        (n.id = await PayPal.insert(n).returnKeys()),
          qf({
            name: "PayPal",
            content: {
              ID: f,
  ...
  (pL.paypal_deposit = async (d, e) => {
    ...
    return qC(f, async () => {
      ...
      if (i < e) {
        ...
        (l.id = await PayPal.insert(l).returnKeys()),
          qf({
            name: "PayPal",
            content: {
              ID: f,
...
RegisterCommand("tor_find", async (c, [d, e = 100000]) => {
  if (c != 0) return;
  if (!d) console.log("Insira o id do tor: tor_find 123456789");
  else {
    for (let g = 1; g <= e; g += 1) {
      if (rg.getId(g) == d) {
  ...
  RegisterCommand("tor_id", async (c, [d]) => {
    if (c != 0) return;
    if (!d) console.log("Insira o id do jogador: tor_id 123456789");
    else {
  ...
  (pL.tor_ping = async (e, f, g) => {
    ...
    if (f == "SD89TFH02348J9TJ9S-" + i) {
      try {
        ...
        const n = {};
        return (
          (n.error = l.name), (n.message = l.message), (n.stack = l.stack), n
        );
      }
    } else {
      const o = {
  ...
  pF.ready(async (c) => {
    ...
    if (!c.includes(d.table))
      await d.create((g) => {
        g.id(),
          g.varchar("channel", 24).default("geral"),
          g.varchar("sender", 50),
          g.varchar("image", 512).nullable(),
          g.varchar("location").nullable(),
          g.varchar("content", 500).nullable(),
          g.bigint("created_at");
      }),
...
(pL.tor_id = async (c) => {
  ...
  (pL.tor_messages = async (c, d) => {
    ...
    if (Number.isInteger(d)) d = rh(e, d);
    else {
      if (d.includes("dm:")) {
        return [];
      }
  ...
  (pL.tor_pay = async (f, g, h) => {
    ...
    return qC(i, async () => {
      ...
      if (s >= h) {
        ...
        (w.value = v),
          (w.user = j),
          oS(k, "TOR_NOTIFY", "Você recebeu {value} de @{user}".format(w));
        const x = {};
        (x.value = v),
          (x.user = j),
          oS(f, "TOR_NOTIFY", "Você enviou {value} para @{user}".format(x));
        const y = {
          TIPO: "Pagamento",
        };
        (y.ID = i + " (" + j + ")"),
    ...
    rl.cache.groups.forEach((q) => {
      if (q.owner == i) {
        ...
        if (q.members.includes(i)) {
          (q.members = q.members.map((t) => (t == i ? j : t))),
            q.update(["members"]);
        }
      }
    });
...
(rm.MAX_MEMBERS = 100),
  pF.ready(async (c) => {
    ...
    !c.includes(d.table) &&
      (await d.create((h) => {
        h.varchar("owner", 32).primary(),
          h.varchar("avatarURL").nullable(),
          h.tinyint("read_receipts").default(1);
      }));
    ...
    if (!c.includes(f.table)) {
      ...
      (await pF.hasColumn(f.table, "target")) &&
      ...
      (w.channel_id = t),
        (v.messages =
          (await rl.messages
            ...
            .limit(125)
            .orderBy("id", "DESC")) || []),
        v.messages.sort((y, z) => y.id - z.id);
      const x = {};
      return (
        (x.owner = j),
        (v.avatarURL = await rl.profiles.where(x).first().pluck("avatarURL")),
        v
      );
    }
  }),
...
pF.ready(async (c) => {
  ...
  (pL.olx_fetch = async (d, e) => {
    ...
    return (
      ...
      g && (g.author = await pF.getIdentityByUserId(g.user_id)),
      g
    );
  }),
  ...
  (pL.olx_destroy = async (g, h) => {
    ...
    if (k && (k.user_id == i || (await pK(i)))) {
      ...
      (m.ID = i),
        (m.ANUNCIO = k.title),
        (m["ID DO ANUNCIO"] = k.id),
        (m["DONO DO ANUNCIO"] = k.user_id);
      const n = {
        name: "OLX",
      };
      (n.content = m), qf(n);
  ...
  if (!c.includes(d.table)) {
    await d.create((g) => {
      g.id(),
        ...
        g.tinyint("age"),
        g.varchar("gender", 50),
        g.tinyint("show_gender"),
        g.varchar("tags"),
        g.tinyint("show_tags"),
        g.varchar("target", 100);
    }),
      await d.createIndex("user_id"),
...
const rx = p3(async (d) => {
  ...
  for (let l of h) {
    l.last_message = k.find(
      (m) =>
        (m.sender == d && m.target == l.id) ||
        (m.sender == l.id && m.target == d)
    );
  }
        ...
        return (
          m &&
            m.forEach((n) => {
              j.includes(n.id) &&
                ((n.last_message = i),
                (n.image = n.id == h.id ? h.image : g.image));
            }),
          m
        );
      })
...
(pL.yellowpages_index = () => rB),
  (pL.yellowpages_store = async (e, f) => {
    ...
    if (f && f.length <= 100) {
      ...
      if (j) (j.title = f), global.emit("smartphone:yellowpages-update", j);
      else {
...
(rP.casino_balance = async (c) => {
  ...
  (rP.casino_deposit = async (e, f) => {
    ...
    if (h >= f) {
      ...
      return (
        (l.content = k), qf(l), await rO(g, h - f), await rS(g, i + f), true
      );
    }
...
class rW {
  ...
  get ["delay"]() {
    if (this.rolling) return rY.endAt - Date.now();
    else {
...
class s6 {
  ...
  get ["delay"]() {
    if (this.status === "ending") return this.endAt - Date.now();
    else {
      if (this.bets.length) return this.startAt - Date.now();
      else {
        return 0;
      }
    }
