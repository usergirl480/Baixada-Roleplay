...
function setTalkingOnRadio(plySource,enabled,identity)
	toggleVoice(plySource,enabled,"radio",identity)
...
RegisterCommand("+radiotalk",function()
	...
	if IsPedSwimming(ped) or exports["player"]:handCuff() then
		return
	end 
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			uObject = CreateObject(GetHashKey('prop_cs_hand_radio'),coords.x,coords.y,coords.z,true,true,true)
			SetEntityCollision(uObject,false,false)
			AttachEntityToEntity(uObject,PlayerPedId(),GetPedBoneIndex(PlayerPedId(),60309),0.06,0.05,0.03,-90.0,30.0,0.0,false,false,false,false,2,true)
