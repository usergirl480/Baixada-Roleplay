...
function toggleVoice(plySource,enabled,moduleType,identity)
	if enabled then
		MumbleSetVolumeOverrideByServerId(plySource,enabled and volumes[moduleType])
		if moduleType then
			TriggerEvent("radio_modules:listRadio",true,identity)
		...
		TriggerEvent("radio_modules:listRadio",false)
...
Citizen.CreateThread(function()
	while true do
		while not MumbleIsConnected() do
			...
			if not Alert then 
				Alert = true
				TriggerEvent("pma-voice:status",Alert)
