...
AddEventHandler("chat:messageEntered",function(message)
	...
	if user_id then
		...
		if identity then
			TriggerClientEvent("chatME",source,"^0"..identity["name"].." "..identity["name2"].." ("..user_id.."): "..message)

			local players = vRPC.nearestPlayers(source,10)
			for _,v in pairs(players) do
				TriggerClientEvent("chatME",v[2],"^0"..identity["name"].." "..identity["name2"].." ("..user_id.."): "..message)
			end
		end
	end
end)
