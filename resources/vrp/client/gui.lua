...
RegisterCommand("cRbind",function(source,args)
	if GetGameTimer() >= cdBtns and playerActive then
		...
		local ped = PlayerPedId()
		if not IsPauseMenuActive() and not LocalPlayer.state.inArena and not exports["inventory"]:blockInvents() and not exports["player"]:blockCommands() and not celular and GetEntityHealth(ped) > 101 and not cancelando then
