...
-- SETBACKPACK	
-----------------------------------------------------------------------------------------------------------------------------------------	
function vRP.giveBackpack(user_id,amount)
	local dataTable = vRP.getDatatable(user_id)
	if dataTable then
		dataTable["backpack"] = dataTable["backpack"] + amount
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SETBACKPACK	
-----------------------------------------------------------------------------------------------------------------------------------------	
function vRP.removeBackpack(user_id)
	local dataTable = vRP.getDatatable(user_id)
	if dataTable then
...
function vRP.checkBroken(nameItem)
	...
	local durability = itemDurability(nameItem)
	if splitName[2] ~= nil and durability then
		if parseInt(os.time() - splitName[2]) >= (86400 * durability) then
			return true
		end
	end
	...
	if parseInt(amount) > 0 then
		...
		if not slot then
			...
			if initial > vRP.getBackpack(user_id) then
				...
				exports["common"]:Log().embedDiscord(user_id,"inventory-invexplode","**ITEM:**```"..itemName(nameItem).."```\n**QUANTIDADE:**```"..parseInt(amount).."```\nDropou às "..os.date("%d/%m/%y %H:%M:%S"),8686827)
...
AddEventHandler("admin:KickAll",function()
	...
	TriggerEvent("reputation:saveSquads")
	TriggerEvent("characters_modules:removeVehicles")

	Citizen.CreateThread(function()
		while true do
			print("[^1Session^7] Finalizado o kick de todos os usuários, pode desligar o servidor.")
			Citizen.Wait(1000)
		end
	end)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		Citizen.Wait(15 * 60000) -- 15 minutos
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVUPDATE
