...
RegisterNUICallback("invClose",function(data)
	...
	if adminMenu then 
		homes["open"],homes["vault"],adminMenu = "","",false
	end
...
function createPed()
	...
	if HasModelLoaded(mHash) then 
		local coords = theftCoords[homes["theft"]] and theftCoords[homes["theft"]].NPC
		npcPed = CreatePed(4,0xE52E126C,coords[1],coords[2],coords[3],274.97,true,false)

		Wait(3000)
        SetPedArmour(npcPed,100)
		GiveWeaponToPed(npcPed,GetHashKey("WEAPON_SNSPISTOL"),250,true,true)
		TaskShootAtEntity(npcPed,ped,25000,GetHashKey("FIRING_PATTERN_FULL_AUTO"))
        SetModelAsNoLongerNeeded(mHash)
		SetPedCombatRange(npcPed,2)
		SetPedCombatMovement(npcPed,2)
		SetPedCombatAbility(npcPed,100)
		SetPedFleeAttributes(npcPed,0,0)
		SetPedCombatAttributes(npcPed,46,1)
...
-- HOMES:ADMINVAULT
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("homes_modules:adminVault",function(homeName)
	homes["open"],homes["vault"],adminMenu = homeName,"vault",true
	TriggerEvent("sounds:source","chest",0.7)
	SendNUIMessage({ action = "showMenu" })
	SetNuiFocus(true,true)
end)
