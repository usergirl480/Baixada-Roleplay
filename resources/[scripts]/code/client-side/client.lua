...
AddEventHandler("notifyShooting",function(coords)
	if policeService then
		TriggerEvent("NotifyPush",{ code = "QRU", title = "Confronto em andamento", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Disparos de arma de fogo", blipColor = 1 })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CLOSESYSTEM
...
RegisterCommand("code",function(source,args)
	if policeService then
		...
		SendNUIMessage({ code = true })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- KEYMAPPING
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterKeyMapping("code","Abrir o código dez","keyboard","f3")
