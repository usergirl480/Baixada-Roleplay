...
AddEventHandler("dismantle:checkVehicle",function(vehicle)
	if not dismantleProgress then
		...
		local vehName = vehicle[2]
		local vehPlate = vehicle[1]
		
		if vSERVER.checkListVeh(vehPlate,vehName) then
			dismantleProgress = true

			SetEntityInvincible(ped,true)
			FreezeEntityPosition(ped,true)
			TriggerEvent("cancelando",true)
			FreezeEntityPosition(vehicle[3],true)
			vRP.playAnim(false,{"anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer"},true)

			for _,v in pairs(vehDoors) do
				local objectExist = GetEntityBoneIndexByName(vehicle[3],v[1])
				if objectExist ~= -1 then
					TriggerEvent("Progress",10000)
					Citizen.Wait(10000)
					SetVehicleDoorBroken(vehicle[3],v[2],false)
				end
			end
			...
			for _,v in pairs(vehTyres) do
				...
				if objectExist ~= -1 then
					TriggerEvent("Progress",10000)
					Citizen.Wait(10000)
					SetVehicleTyreBurst(vehicle[3],v[2],1,1000.01)
				end
			end
			...
			vSERVER.paymentMethod(vehicle[3])
