...
RegisterCommand("cor",function(source,args)
	if vSERVER.permissionVIP() then
		...
		if weaponColor >= 0 then
			...
			TriggerEvent("Notify","negado","Você precisa digitar uma numeração de cor válida.",10000,"normal")
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		local timeDistance = 999
		local ped = PlayerPedId()
		if IsPedArmed(ped,6) and GetGameTimer() >= sprayTimers and not LocalPlayer.state.inArena then
			timeDistance = 1
...
AddEventHandler("player:enterTrunk",function()
	...
	if not IsPedInAnyVehicle(ped) then
		if not inTrunk then
			...
			if vSERVER.vehicleTrunkLimit(vehPlate) then
					...
					if trunk ~= -1 and speed <= 3 then
						...
						if distance <= 3.0 then
							if GetVehicleDoorAngleRatio(vehicle,5) < 0.9 and GetVehicleDoorsLockedForPlayer(vehicle,PlayerId()) ~= 1 then
								...
								blockCommands = true
								inTrunk = true
								vSERVER.increaseVehicleTrunk(vehPlate)
							end
						end
					end
				end
...
-- AGRESSIVEPED
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.agressivePed(hasPed)
	local ped = PlayerPedId()
	...
	GiveWeaponToPed(hasPed,GetHashKey("WEAPON_SNSPISTOL"),250,true,true)
...
-- ENDGAME
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("player:endGame")
AddEventHandler("player:endGame",function()
	TriggerServerEvent("vRP:endGame")
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- BXD-CARRY
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("bxd:carry",function(source,args)
	if not args[1] then return end

	TriggerServerEvent("player:carryFunctions",args[1])
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- BXD-HANDCUFF
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("bxd:handcuff",function(source,args)
	TriggerServerEvent("player:handcuffPlayer")
end)

RegisterCommand("bxd:identity",function(source,args)
	TriggerServerEvent("player:identityFunctions")
end)

RegisterKeyMapping("bxd:carry bracos","Carregar nos bracos","keyboard","H")
RegisterKeyMapping("bxd:handcuff","Algemar","keyboard","G")
RegisterKeyMapping("bxd:identity","Identitdade","keyboard","F11")
