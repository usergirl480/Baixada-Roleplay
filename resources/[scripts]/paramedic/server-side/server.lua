...
AddEventHandler("paramedic:tratamento",function(entity)
	...
	if user_id and vRP.getHealth(source) > 101 and vRP.getHealth(entity) > 101 then
		if exports["common"]:Group().hasPermission(user_id,"paramedic") then
			...
			TriggerClientEvent("Notify",source,"importante","Tratamento começou.")
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("paramedic:sangramento",function(entity)
	...
	if user_id and vRP.getHealth(source) > 101 and vRP.getHealth(entity) > 101 then
		if exports["common"]:Group().hasPermission(user_id,"paramedic") then
			TriggerClientEvent("resetBleeding",entity)
			TriggerClientEvent("Notify",source,"importante","Sangramento parou.")
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("paramedic:diagnostico",function(entity)
	...
	if user_id and vRP.getHealth(source) > 101 then
		if exports["common"]:Group().hasPermission(user_id,"paramedic") then
			...
			if text ~= "" then
				TriggerClientEvent("Notify",source,"importante","Status do paciente:<br>" .. text)
			elseif not hurt then
				TriggerClientEvent("Notify",source,"sucesso","Status do paciente:<br>- <b>Nada encontrado</b>")
			end
		end
	end
end)
