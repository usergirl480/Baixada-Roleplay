        ...
        if vehicle ~= 0 then
            if vSERVER.permissionVIP() then
                ...
                if vehicleSpeed <= 25 then
                    ...
                    TriggerEvent("Notify","negado","Você está muito rápido!",5000)
                end
            else
                TriggerEvent("Notify","negado","Acesso negado!",5000)
            end
        else
...
RegisterNUICallback("ButtonClick", function(data, cb)
    ...
    if data.action == "useControl" then
        ...
        if alturaAtual > tonumber(0.09) then
            TriggerEvent("Notify", "negado", "Altura máxima atingida!", 5000)
            return
        end
        if alturaAtual < tonumber(-0.09) then
            TriggerEvent("Notify", "negado", "Altura miníma atingida!", 5000)
            return
        end
            ...
            if vehicleSpeed == 0 then
                if vSERVER.checkPermission() then
                    if not vSERVER.checkXenon() then
                        ...
                        vSERVER.installXenon(vehicle,GetVehicleNumberPlateText(vehicle),vRP.vehicleName())
                    else
                        TriggerEvent("Notify", "negado", "Veículo já possui módulo de Xenon", 5000)
                    end
                else
                    TriggerEvent("Notify", "negado", "Você não possui permissão para instalar o módulo de Xenon!", 5000)
                end
            else
                TriggerEvent("Notify", "negado", "O veículo deve estar parado!", 5000)
            end
        else
            TriggerEvent("Notify", "negado", "Você não é o motorista do veículo", 5000)
        end
    else
        TriggerEvent("Notify", "negado", "Você não está em um veículo!", 5000)
    end
            ...
            if vehicleSpeed == 0 then
                if vSERVER.checkPermission() then
                    if not vSERVER.checkNeon() then
                        SetVehicleEngineOn(vehicle, false, true, true)
                        vSERVER.installNeon(vehicle,GetVehicleNumberPlateText(vehicle),vRP.vehicleName())
                    else
                        TriggerEvent("Notify", "negado", "Veículo já possui módulo de neon", 5000)
                    end
                else
                    TriggerEvent("Notify", "negado", "Você não possui permissão para instalar o módulo de neon!", 5000)
                end
            else
                TriggerEvent("Notify", "negado", "O veículo deve estar parado!", 5000)
            end
...
AddEventHandler('zo_install_suspe_ar', function()
    if vSERVER.checkPermission() then
        local ped = PlayerPedId()
        vehicleInstall = vRP.nearVehicle(5)
                                                ...
                                                while true do
                                                    ...
                                                    if vehicleInstall == nil or #partsProntas == 4 then
                                                        if vehicleInstall == nil then
                                                            TriggerEvent("Notify", "negado", "Instalação cancelada, você não está próximo do veículo!", 5000)
                                                        end
                                                        return
                                                    end
                                                end
...
-- 				DrawMarker(0, v.x, v.y, v.z - 0.40, 0, 0, 0, 0, 0, 0, 0.7, 0.7, 0.5, 136, 96, 240, 180, 1, 0, 0, 1)
-- 				idle = 5
