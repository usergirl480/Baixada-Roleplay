...
Citizen.CreateThread(function()
	...
	AddCircleZone("systemHacker",vector3(-195.44,-835.2,30.72),0.8,{
		name = "systemHacker",
		heading = 25.52
	},{
		distance = 0.75,
...
local distance_zones = {
	["dismantle"] = {
		vec3(947.07,-1697.93,30.08),
		vec3(964.21,-1856.54,31.19),
		vec3(1261.15,-2565.92,42.71),
		vec3(146.73,320.62,112.14),
		vec3(-83.03,369.05,112.46),
		vec3(2357.31,3132.24,48.21),
		vec3(258.65,2589.41,44.96),
		vec3(2135.24,4778.29,40.96),
		vec3(2195.95,5606.64,53.53),
		vec3(2330.47,2572.62,46.67),
	},
}
...
local function checkDistance(index) 
	...
	if distance_zones[index] then 
		for _,v in pairs(distance_zones[index]) do
			if #(coords - v) then
				return true
			end
		end
					...
					elseif IsEntityAVehicle(entity) then
						if #(coords - entCoords) <= 1.0 then
							...
							if policeService then
								...
								if not checkDistance('dismantle') then
